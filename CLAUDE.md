...
| `Family` | `FamilyScreen.js` | Currently a voice-assistant test harness (continuous mic loop, manual TTS test buttons), not a family/meal-calendar feature — its own heading renders `t('coming_soon')`. PRO-gated with a non-dismissable `UpgradeGateModal` (`onClose` is a no-op) for non-PRO users. |
