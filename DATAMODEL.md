...
- **`pull()`** — POSTs `{ action: 'pull', userId, token }` to `sync`. Takes `result.data` (`saved`, `meal_plan`, `shopping`, `books`, `activities`, `followed_bloggers`, `remi_explicit`, `user_stores`, `available_coupons`, `coupons` — the wallet, see the `rv4_wallet_coupons` naming-trap note below — `cb_covers`, and `tier`), runs an emoji-corruption repair pass over `saved` (fixes `�`-corrupted `recipe.emoji` fields), maps it into the `data` shape above, calls `setData(next)`, then writes **both** the combined object to `fern_sync_cache` **and** each field out to its own individual key (`rv4_saved`, `rv4_books`, `rv4_meal_plan`, `rv4_master_shop`, `remi_explicit`, `cpc_followed_bloggers`, `cpc_user_stores`, `rv4_available_coupons`, `rv4_wallet_coupons`, `rv4_activities`, `rv4_cb_covers`, `fern_user_tier`). If the emoji repair changed anything, it immediately pushes the fix back to the server (including `wallet_coupons`, `activities`, `cb_covers`, and `tier`, so that push doesn't clobber any of them — `available_coupons` is intentionally excluded from every push, see below).
...
| `rv4_available_coupons` | Array of raw coupon objects from the server's catalog. Confirmed shape from a live pull sample: `{ id, code, image, store, title, expiry, source, category, discount, redemption, description }` (`redemption` is `'link'`/`'digital'`/`'code'`; `store` can be `''`; `image` can be `null`; `expiry` is an ISO datetime string, or `null` for some `retailmenot.com`-sourced entries). `src/utils/couponNormalize.js`'s `normalizeCoupon` still reads several aliases per field (`discountLabel`/`badge`/`discount`/`savings`/`offerLabel`, etc.) as a defensive fallback, since not every field has been cross-checked against every `source`. | `useSync.pull()`; `useAuth.syncPull` | `useSync.js` (as `data.availableCoupons`); `HomeScreen.js`/`CouponWalletScreen.js` (Browse tab) | Pull-only — refreshed wholesale on every pull, never part of any push (it's the server's catalog, not user data) |
| `rv4_wallet_coupons` | Array of coupon objects (same shape as `rv4_available_coupons`) the user has added to their wallet. **Naming trap:** the pull response's real key for this is `coupons`, not `wallet_coupons` — confirmed from a live pull sample where `wallet_coupons` was absent entirely and `coupons` held the actual populated wallet. `useSync.pull()`/`useAuth.syncPull` read `dd.coupons \|\| dd.wallet_coupons` (fallback kept in case an older/alternate response shape exists). The **push** side still sends the field as `wallet_coupons` (unconfirmed against a real push response — left as-is since that part was already working) — so the pull key and push key genuinely differ, don't "fix" one to match the other without re-confirming against the live API first. | `HomeScreen.js` (`addCouponToWallet`/`removeCouponFromWallet`); `useSync.pull()`; `useAuth.syncPull` | `useSync.js` (as `data.walletCoupons`); `HomeScreen.js`/`CouponWalletScreen.js` (My Wallet tab, and the Home "Coupons" stat count) | Synced — pushed as `wallet_coupons` via `pushChangedFromStorage`, pulled back as `coupons` |
