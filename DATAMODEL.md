...
data: {
  ...
  books: [],            // ← rv4_books
  activities: [],        // ← rv4_activities
  followers: [],          // ← cpc_followed_bloggers
  userProfile: {},         // ← remi_explicit
  userStores: [],           // ← cpc_user_stores
  walletCoupons: [],          // ← coupons (confirmed from a live pull sample — NOT wallet_coupons, despite the push field/AsyncStorage key both being named wallet_coupons; see below)
  cbCovers: {},                // ← cb_covers (cookbook cover-photo URLs keyed by book id, e.g. { "cb_1786384300067": "https://...jpg" })
  tier: 'free',                 // ← tier ('free' | 'pro' | 'pro_max' — see below)
}
```

- **`pull()`** — POSTs `{ action: 'pull', userId, token }` to `sync`. Takes `result.data` (`saved`, `meal_plan`, `shopping`, `books`, `activities`, `followed_bloggers`, `remi_explicit`, `user_stores`, `coupons` — the wallet, see the `rv4_wallet_coupons` naming-trap note below — `cb_covers`, and `tier`), runs an emoji-corruption repair pass over `saved` (fixes `�`-corrupted `recipe.emoji` fields), maps it into the `data` shape above, calls `setData(next)`, then writes **both** the combined object to `fern_sync_cache` **and** each field out to its own individual key (`rv4_saved`, `rv4_books`, `rv4_meal_plan`, `rv4_master_shop`, `remi_explicit`, `cpc_followed_bloggers`, `cpc_user_stores`, `rv4_wallet_coupons`, `rv4_activities`, `rv4_cb_covers`, `fern_user_tier`). If the emoji repair changed anything, it immediately pushes the fix back to the server (including `wallet_coupons`/`coupons`, `activities`, `cb_covers`, and `tier`, so that push doesn't clobber any of them). **`available_coupons`, if present in the response, is intentionally ignored** — the client no longer reads or stores it (no `rv4_available_coupons` key exists anymore); the full coupon catalog is instead fetched live from the separate `fetch-coupons` function (see API reference) every time the Coupon Wallet's Browse tab opens.
...
- **`pushChangedFromStorage(changedData = {})`** — reads the 11 canonical keys fresh off disk (the original 7 plus `rv4_wallet_coupons`, `rv4_activities`, `rv4_cb_covers`, and `fern_user_tier`), builds `data: { saved, books, meal_plan, shopping, remi_explicit, followed_bloggers, user_stores, wallet_coupons, coupons, activities, cb_covers, tier, ...changedData }` — `wallet_coupons` and `coupons` are both set to the same array (see the `rv4_wallet_coupons` naming-trap note below) — (so in-memory `changedData` fields override what's on disk), POSTs `{ action: 'push', userId, token, data }`. This is what screens call after any local mutation.
- **`pushAllFromStorage()`** — `pushChangedFromStorage()` with no overrides, i.e. "push whatever's currently on disk."
- **Gap:** there is no push path for partial `userProfile` fields beyond the whole `remi_explicit` blob. Only the wallet coupons (pushed under both `wallet_coupons` and `coupons`), `activities` (the user's Family Hub activities), `cb_covers` (cookbook cover photos), and `tier` (the user's subscription plan) are round-tripped — the coupon catalog itself (`fetch-coupons`) is a read-only, unsynced live fetch, not part of push at all.

### Subscription tier sync (`tier`)

`tier` is `'free' | 'pro' | 'pro_max'` (matches `src/constants/tiers.js`'s `TIERS`), synced through the same `rv4_*`-style pattern as everything else, under the AsyncStorage key `fern_user_tier` and the pull/push field `tier`. RevenueCat (`src/services/RevenueCatContext.js`) is the on-device source of truth for the tier — the sync `tier` field exists so the **web app** (which has no RevenueCat SDK on its own) can read the same subscription status server-side, not so the native app can read it back (the app should always ask `useEntitlement()`/`useRevenueCat()` for gating decisions, never `data.tier` from `useSync`). `App.js`'s `MainAppContent` pushes the current RevenueCat tier via `pushChangedFromStorage({ tier })` on every app launch, once `user` is authenticated and RevenueCat's own `loading` flag has resolved (pushing while RevenueCat is still loading would push its temporary `FREE` fallback and clobber a real Pro/Pro Max tier server-side) — and again whenever `tier` changes mid-session (e.g. right after a purchase). This is a client-push design: the source of truth is Apple/Google's receipt validated by RevenueCat, mirrored into the backend on a best-effort basis: there's no webhook from RevenueCat into `clickpickandcook.com` in this repo, so the backend's copy of `tier` is only as fresh as the last time this app was opened by that user.
...
1. **It never writes `fern_sync_cache`.** Only the individual keys get written, so immediately post-login/signup the combined cache is stale until `useSync`'s own `pull()` fires on the next render.
...
4. `syncPull` does write `fern_user_tier` (kept in sync with `useSync.pull()` when `tier` was added) but, like `cb_covers`, does **not** write `rv4_cb_covers` — see that key's row in the table below.
...
| `rv4_wallet_coupons` | Array of coupon objects (same shape as the `fetch-coupons` catalog, see API reference) the user has added to their wallet. **Naming trap:** the pull response's real key for this is `coupons`, not `wallet_coupons` — confirmed from a live pull sample where `wallet_coupons` was absent entirely and `coupons` held the actual populated wallet. `useSync.pull()`/`useAuth.syncPull` read `dd.coupons \|\| dd.wallet_coupons`. Since a user reported added coupons not surviving a relaunch (count/wallet reverting after `pull()`), which pointed at the push side silently not persisting under `wallet_coupons` if the backend's writable field is actually `coupons`, every push of this field (`useSync`'s `pushChangedFromStorage` base payload, its emoji-repair auto-push, and `HomeScreen.js`'s `addCouponToWallet`/`removeCouponFromWallet`) now sends **both** `wallet_coupons` and `coupons` with the same array — still unconfirmed which key the backend actually reads, so this covers either case rather than guessing one. `addCouponToWallet`/`removeCouponFromWallet` also patch `fern_sync_cache`'s `walletCoupons` field directly (matching the same-tick cache-patch pattern every other synced mutation uses), so a relaunch before the next `pull()` completes hydrates the correct wallet instead of the stale cached one. | `HomeScreen.js` (`addCouponToWallet`/`removeCouponFromWallet`); `useSync.pull()`; `useAuth.syncPull` | `useSync.js` (as `data.walletCoupons`); `HomeScreen.js`/`CouponWalletScreen.js` (My Wallet tab, and the Home "Coupons" stat count) | Synced — pushed as both `wallet_coupons` and `coupons` via `pushChangedFromStorage`, pulled back as `coupons` |
| `rv4_activities` | Array of `{ day, time, emoji, label, dateKey, startTime, endTime, _origLabel }` — `dateKey` is `YYYY-MM-DD` (matches `rv4_meal_plan`'s keying), `day` is a display label like `"Sun 9"`, `time` is `"{startTime} - {endTime}"` when both are set (else just `startTime`, else empty). `startTime`/`endTime` are formatted `"H:MM AM/PM"` strings, or empty if not set. | `FamilyScreen.js` (`FamilyAddActivityModal` add action); `useSync.pull()`; `useAuth.syncPull` | `useSync.js` (as `data.activities`); `FamilyScreen.js` (Family Hub's per-day Activities list and the `family_hub_stat_activities` count) | Synced — pushed as `activities` via `pushChangedFromStorage` |
| `rv4_cb_covers` | Object keyed by cookbook id → cover-photo URL, e.g. `{ "cb_1786384300067": "https://axqkdhzcfcroxlkducea.supabase.co/storage/v1/object/public/recipe-photos/cover_cb_....jpg" }`. The photo itself is uploaded separately via the `upload-photo` function (see API reference), which returns the URL stored here — this key never holds base64. `RecipesScreen.js` resolves a book's cover as `data.cbCovers[book.id] || book.cover` (the book object's own `cover`/`image` field is only a same-session fallback for instant display before the first push/pull round-trip). **Not yet added to `useAuth.syncPull`** — matches the existing documented divergence between `useSync.pull()` and `useAuth.syncPull()` above; a user who logs in fresh won't see cookbook covers until `useSync`'s own `pull()` fires. | `RecipesScreen.js` (`NewCookbookModal` create action); `useSync.pull()` | `useSync.js` (as `data.cbCovers`); `RecipesScreen.js` (cookbook shelf + detail header cover images) | Synced — pushed as `cb_covers` via `pushChangedFromStorage`, pulled back as `cb_covers` |
| `fern_user_tier` | Plain JSON string, one of `'free'` \| `'pro'` \| `'pro_max'` (`src/constants/tiers.js`'s `TIERS`). Written from RevenueCat's resolved entitlement (`tierFromCustomerInfo`, `src/constants/revenuecat.js`), **not** from the sync pull response driving the app's own gating — see the "Subscription tier sync" note above for why this key exists (mirroring status to the web app) vs. `useEntitlement()`/`useRevenueCat()` (the actual gating source of truth). | `App.js`'s `MainAppContent` (on every app launch and on tier change, via RevenueCat); `useSync.pull()`; `useAuth.syncPull` (as a fallback copy of whatever the server last had) | `useSync.js` (as `data.tier`); `pushChangedFromStorage()` (read back off disk to carry the last-known tier forward on unrelated pushes) | Synced — pushed as `tier` via `pushChangedFromStorage`, pulled back as `tier` |
...
| `rv4_family_vault` | Array of `{ id: 'family-vault-<ts>', title, origin, story, ingredients: [""], instructions: [""], createdAt }`, newest first. Populated via three entry points inside `FamilyRecipeVaultModal` — type it in directly, import from a URL, or scan a photographed recipe card — all funneling through the same add form before being appended here. Distinct from `rv4_saved`: a vault recipe only lands in the regular recipe library (`rv4_saved`) if the user explicitly taps "Save to My Recipes" from its detail view (via `useAiRecipeCollection.saveToLibrary`). | `FamilyRecipeVaultModal.js` (`HomeScreen.js`'s "Family Vault" tool tile) | `FamilyRecipeVaultModal.js` | Device-only — no confirmed server-side field for this yet, so it does not survive reinstall/relogin on another device |
...
**Removed key:** `rv4_available_coupons` no longer exists — the coupon catalog it used to cache (sync's `available_coupons` field) is not collected by `useSync.pull()`/`useAuth.syncPull` anymore. The Coupon Wallet's Browse tab now fetches the live catalog directly from the `fetch-coupons` function on every open instead (see API reference) — nothing writes this key going forward, though a stale copy may still linger in AsyncStorage on devices that had it from before this change.
...
| `familyVaultService.tellRecipeStory` | *(none)* | Plain text — a short 2-4 sentence story paragraph. Caller: `FamilyRecipeVaultModal`'s "Let Fern help tell the story" button, seeded from the in-progress title/origin fields. |
| `familyVaultService.importRecipeFromUrl` | `recipe_search` | `{"title","origin","story","ingredients":[""],"instructions":[""]}` — asks the AI to fetch and parse a recipe page from a URL; best-effort, since this app has no confirmation that the backend can actually browse a live URL (untested against a real response as of this writing — the caller degrades gracefully to an "Import Failed" alert and lets the user type the recipe in manually). Caller: `FamilyRecipeVaultModal`'s Import URL flow. |
| `familyVaultService.scanRecipeCard` | `recipe_search` | `{"title","origin","story","ingredients":[""],"instructions":[""]}` — transcribes a photographed recipe card. Sends the photo as a vision `image` content block (`{type:'image', source:{type:'base64', media_type, data}}`) alongside the text prompt in `messages[0].content`, following the Anthropic Messages API convention this endpoint's `content: [{type:'text',...}]` response shape implies; **unconfirmed against a real multimodal response** since no other caller in this app sends an image this way — degrades gracefully to a "Scan Failed" alert if parsing comes back empty. `mode` (`full`/`ingredients`/`instructions`) adjusts the system prompt so only the relevant fields are requested. Caller: `FamilyRecipeVaultModal`'s Scan Card flow (camera or photo library). |
...
Payload: `{ photoBase64, fileName }` — `photoBase64` is a full data URI (`data:image/jpeg;base64,...`), not bare base64; `fileName` is caller-generated (no `token`/`userId` in the payload). Response: `{ success, url }`, where `url` points at Supabase storage (`https://axqkdhzcfcroxlkducea.supabase.co/storage/v1/object/public/recipe-photos/<fileName>`). Caller: `src/services/uploadPhotoService.js`'s `uploadPhoto`, used by `RecipesScreen.js`'s `NewCookbookModal` to upload a cookbook cover photo — the returned `url` is stored in `rv4_cb_covers` (see local storage key table), not on the book object itself.
...
One of two **GET** endpoints (the other is `fetch-coupons`, below), no body. Response: `{ lat, lon, address, source }`, validated and returned as `{ lat, lon, lng: lon, address, source }` (or `null` on failure). Caller: `src/services/storeLookupService.js`'s `findStoreLocationByZip`, used by `HomeScreen.js`'s store-management UI.

### `fetch-coupons` — `GET /.netlify/functions/fetch-coupons`

The other **GET** endpoint — no query params, no body, no `token`/`userId`. Response: `{ coupons: [{ id, code, image, store, title, expiry, source, category, discount, redemption, description }] }` — confirmed live (102 items at time of writing): `redemption` is `'digital'`/`'link'`/`'code'`, `store` can be `''`, `image` can be `null`, `expiry` is an ISO datetime string. This is the full coupon catalog — same shape the sync payload's now-unused `available_coupons` field used to carry (see the removed-key note above). Caller: `src/services/couponsService.js`'s `fetchAllCoupons`, called by `HomeScreen.js`'s `openCouponWallet` every time the Coupon Wallet's Browse tab is opened — not cached or synced, always a live fetch. Normalized client-side via `src/utils/couponNormalize.js`'s `normalizeCoupon`/`normalizeCoupons`. Contrast with `rv4_wallet_coupons`/`data.walletCoupons` (the user's *saved* coupons, sourced from the `sync` function's `coupons` field, not this endpoint) — Browse shows the live catalog from here, My Wallet shows the synced wallet array.
